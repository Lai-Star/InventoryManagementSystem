...
func ProductNameFormValidation(w http.ResponseWriter, productName, action string) bool {
	if action == "CREATE" {
		...
		if utils.IsBlankField(productName) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Name cannot be empty. Please try again.")
			return false
		}
	}
	...
	if action == "CREATE" || (action == "UPDATE" && len(productName) > 0) {
		...
		if !utils.CheckLengthRange(productName, 1, 255) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Name must have a length of 1 - 255 characters.")
			return false
		}
	}
...
func ProductSkuFormValidation(w http.ResponseWriter, productSku, action string) bool {
	if action == "CREATE" {
		...
		if utils.IsBlankField(productSku) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Sku cannot be empty. Please try again.")
			return false
		}
	}
	...
	if action == "CREATE" || (action == "UPDATE" && len(productSku) > 0) {
		...
		if !utils.CheckLengthRange(productSku, 1, 50) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Sku must have a length of 1 - 50 characters.")
			return false
		}
	}
...
func ProductBrandFormValidation(w http.ResponseWriter, productBrand, action string) bool {
	if action == "CREATE" {
		...
		if utils.IsBlankField(productBrand) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Brand cannot be empty. Please try again.")
			return false
		}
	}
	...
	if action == "CREATE" || (action == "UPDATE" && len(productBrand) > 0) {
		...
		if !utils.CheckLengthRange(productBrand, 1, 50) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Brand must have a length of 1 - 50 characters.")
			return false
		}
	}
...
func ProductColourFormValidation(w http.ResponseWriter, productColour, action string) bool {
	if action == "CREATE" {
		...
		if utils.IsBlankField(productColour) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Colour cannot be empty. Please try again.")
			return false
		}
	}
	...
	if action == "CREATE" || (action == "UPDATE" && len(productColour) > 0) {
		...
		if !utils.CheckLengthRange(productColour, 1, 7) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Colour must have a length of 1 - 7 characters.")
			return false
		}
	}
...
func ProductCategoryFormValidation(w http.ResponseWriter, productCategory, action string) bool {
	if action == "CREATE" {
		...
		if utils.IsBlankField(productCategory) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Category cannot be empty. Please try again.")
			return false
		}
	}
	...
	if action == "CREATE" || (action == "UPDATE" && len(productCategory) > 0) {
		...
		if !utils.CheckLengthRange(productCategory, 1, 20) {
			utils.WriteJSON(w, http.StatusBadRequest, "Product Category must have a length of 1 - 20 characters.")
			return false
		}
	}
...
func ProductCostFormValidation(w http.ResponseWriter, productCost float32) bool {
	...
	if productCost == 0 {
		utils.WriteJSON(w, http.StatusBadRequest, "Product Cost cannot be 0. Please try again.")
		return false
	}
	...
	if !isValidProductCost {
		utils.WriteJSON(w, http.StatusBadRequest, "Product Cost must have a maximum of 8 digits and 2 decimal places.")
		return false
	}
	return true
}
...
func SizeNameQuantityFormValidation(w http.ResponseWriter, sizes []Size) bool {
	for idx, size := range sizes {
		...
		if utils.IsBlankField(size.SizeName) {
			utils.WriteJSON(w, http.StatusBadRequest, "Size Name cannot be empty. Please try again.")
			return false
		}

		// Check if size name is within 1 and 5 characters
		if !utils.CheckLengthRange(size.SizeName, 1, 5) {
			utils.WriteJSON(w, http.StatusBadRequest, "Size Name must be within 1 - 5 characters. Please try again.")
			return false
		}
		...
		if kind := reflect.TypeOf(size.SizeQuantity).Kind(); kind != reflect.Int &&
			...
			utils.WriteJSON(w, http.StatusBadRequest, "Size Quantity must be an integer value. Please try again.")
			return false
		}

		// Check if size quantity is negative
		if size.SizeQuantity < 0 {
			utils.WriteJSON(w, http.StatusBadRequest, "Size Quantity cannot be negative. Please try again.")
			return false
		}
...
func ValidateAndInsertSize(w http.ResponseWriter, sizes []Size, productId int32) (bool, []Size) {
	...
	for _, size := range sizes {
		...
		if !isValidSizeName {
			utils.WriteJSON(w, http.StatusBadRequest, "Size name must have a length of 0 - 5 characters. Please try again!")
			return false, sizes
		}
		...
		if !isValidSize {
			utils.WriteJSON(w, http.StatusBadRequest, size.SizeName+" is not a valid size name. Please try again.")
			return false, sizes
		}
		...
		if reflect.TypeOf(size.SizeQuantity).Kind() != reflect.Int {
			utils.WriteJSON(w, http.StatusBadRequest, strconv.Itoa(size.SizeQuantity)+" is not in the correct Integer format. Please try again!")
			return false, sizes
		}
